...
function AddQuizForm() {
  ...
  const [questions, setQuestions] = useState([{ question: "", answers: [{ answer: "", isCorrect: false }, { answer: "", isCorrect: false }, { answer: "", isCorrect: false }, { answer: "", isCorrect: false }] }]);
    ...
    <form onSubmit={handleSubmit}>
      ...
      {questions.map((question, questionIndex) => (
        <div key={questionIndex}>
          <label>
            Question:
            <input type="text" name="question" value={question.question} onChange={(event) => handleQuestionChange(event, questionIndex)} />
          </label>
          {question.answers.map((answer, answerIndex) => (
            <div key={answerIndex}>
              <label>
                Answer:
                <input type="text" name="answer" value={answer.answer} onChange={(event) => handleAnswerChange(event, questionIndex, answerIndex)} />
              </label>
              <label>
                Correct:
                <input type="checkbox" checked={answer.isCorrect} onChange={(event) => handleCheckboxChange(event, questionIndex, answerIndex)} />
              </label>
            </div>
          ))}
        </div>
